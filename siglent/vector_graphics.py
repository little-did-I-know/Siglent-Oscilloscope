...
class VectorPath:
    ...
    def rotate(self, angle_degrees: float, origin: Tuple[float, float] = (0, 0)) -> 'VectorPath':
        ...
        # Translate back
        return VectorPath(
            x=x_rotated + origin[0],
            y=y_rotated + origin[1],
            connected=self.connected
        )
    ...
    def combine(self, other: 'VectorPath') -> 'VectorPath':
        ...
        return VectorPath(
            ...
            connected=False  # Combined paths are typically not closed
        )
...
class Shape:
    ...
    @staticmethod
    def star(num_points: int = 5, outer_radius: float = 1.0, inner_radius: float = 0.4,
             center: Tuple[float, float] = (0, 0), points_per_line: int = 50) -> VectorPath:
    ...
    @staticmethod
    def text(text: str, font_size: float = 0.5, position: Tuple[float, float] = (0, 0),
             samples_per_unit: int = 200) -> VectorPath:
...
class VectorDisplay:
    ...
    def save_waveforms(self, path: VectorPath, filename_prefix: str, sample_rate: float = 1e6,
                       duration: float = 0.1, format: str = 'csv'):
